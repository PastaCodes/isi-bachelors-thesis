...
\chapter{Applicazione a dati reali}

Il Minor Planet Center raccoglie dati su osservazioni ottiche astrometriche di corpi minori, comete e satelliti naturali, effettuate da osservatori in tutto il mondo. I dati sono forniti senza alcun tipo di correzione, come specificato nelle linee guida (\url{https://minorplanetcenter.net/iau/info/Astrometry.html#corrs}). Questo li rende difficili da utilizzare direttamente. In effetti, si comincerà considerando un approccio ``naïve'', procedendo poi con l'applicazione del filtro di Kalman ed evidenziando la precisione aggiuntiva ottenuta.
...
Se si conosce la distanza del corpo $\Delta$, diventa possibile determinarne la posizione dagli angoli $\alpha,\delta$ tramite la formula~\ref{eq:radec2pos}. Dunque si vuole risalire a $\Delta$ dalla magnitudine visuale $V$, idealmente sfruttando la formula~\ref{eq:hg-mag}. Tuttavia non si dispone della distanza dal Sole $d$ e dell'angolo di fase $\phi$. Una possibile soluzione prevede di risolvere in $\phi$ la seguente equazione tramite metodi numerici:
\begin{equation}
...
Il vantaggio di questa specifica formulazione sta nel fatto che l'espressione ha valore positivo finito per $\phi=0$, ovvero $R^2\sin^2(\theta)$, e ha valore negativo finito per $\phi\to\pi$, ovvero $-R^2\sin^2(\theta)$, il che permette di utilizzare il metodo di Brent nell'intervallo $[0,\pi]$. \\
