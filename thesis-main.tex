...
EKF~\cite{WelchB95} nasce dalle primissime applicazioni delle intuizioni di Kálmán e Bucy al programma Apollo~\cite{smith1962}. Si basa sull'utilizzo naturale del polinomio di Taylor di grado 1 per trovare approssimazioni lineari alle equazioni del sistema, in generale non lineari. \\
Partendo dalla formula generale del polinomio in una variabile reale:
\begin{equation*}
f(x)=f(a)+f^\prime(a)(x-a)+o(x-a)
\end{equation*}
Applicandola alla legge di evoluzione $f$ di un sistema dinamico:
\begin{equation*}
f(k,x_{k-1},u_{k-1})\approx f(k,\hat{x}_{k-1},u_{k-1})+J_x\,f(k,\hat{x}_{k-1},u_{k-1})(x_{k-1}-\hat{x}_{k-1})
\end{equation*}
In questo caso, $f^\prime(a)$ è sostituita da $J_x\,f(k,\hat{x}_{k-1},u_{k-1})$, ossia la matrice jacobiana delle derivate parziali di $f$ rispetto ad $x$ calcolate in $k,\hat{x}_{k-1},u_{k-1}$. \\
...
\begin{gather}
h(k,x_k)\approx h(k,\hat{x}_k)+J_x\,h(k,\hat{x}_k)(x_{k-1}-\hat{x}_{k-1}) \\
H_k=J_x\,h(k,\hat{x}_k)\Rightarrow\bigl(H_k\bigr)_{ij}=\left.\dfrac{\partial h_i}{\partial x_j}\right\vert_{k,\hat{x}_k}
\end{gather}
...
\end{gather*}
Una volta determinate le matrici jacobiane $A_k$ e $H_k$, EKF fa uso delle stesse formule della controparte lineare per calcolare $P_k^-$,$P_k$ e $K_k$. Invece, dove possibile, ossia nel calcolo delle stime, utilizza direttamente le funzioni $f$ e $h$:
\begin{gather*}
...
Determinato il parametro intermedio $\lambda=\alpha^2(n+\kappa)-n$, TODO...
